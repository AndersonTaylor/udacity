...
def cfgempty(grammar,symbol,visited):
    ...
    if symbol in visited: 
        #in a loop so return None:
        return None
    elif not any( [rule[0] == symbol for rule in grammar] ): 
        #if the symbol does not have a rewrite rule in the
        #grammar then it is a terminal, return it (as a list)
        return [symbol]
    else:
        newvisited = visited + [symbol]
        #each rule is of the form SYMBOL -> RHS
